...
- **7 implemented features** — `Merge` (pypdf), `Sort` (pypdf + PyMuPDF), `Edit & Sign` (PyMuPDF), `Split` (PyMuPDF), `Extract Text` (PyMuPDF), `Extract Image` (PyMuPDF), `Export Excel` (PyMuPDF + openpyxl), `Compress` (PyMuPDF).
